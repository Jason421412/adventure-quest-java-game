...
import entity.*;
import main.util.Assets;                   // ✅ 使用资源加载工具
...
public class Dungeon {
    ...
    private void generateDungeon() {
        ...
        for (int i = 0; i < rooms.length; i++) {
            ...
            for (Monster monster : monsters) {
                monster.setPosition(rand.nextInt(48, GamePanel.SCREEN_WIDTH - 50),
                        rand.nextInt(GamePanel.SCREEN_HEIGHT - 50));
    ...
    private Monster[] generateMonsters() {
        ...
        switch (difficulty) {
            case "Easy":
                num = 1; hp = -20; attackPower = -10; break;
            case "Medium":
                num = 2; hp = 0;   attackPower = 0;    break;
            case "Difficult":
                num = 3; hp = 15;  attackPower = 15;   break;
            default:
    ...
    private Monster generateMonster(int hp, int attackPower){
        ...
        switch(monsterType){
            case 0: return new Goblin(hp, attackPower);
            case 1: return new Skeleton(hp, attackPower);
            case 2: return new Zombie(hp, attackPower);
    ...
    // Load a random floor background from available resources
    public BufferedImage generateBackgroundImage() {
        ...
        String pick = paths[rand.nextInt(paths.length)];
        try {
            return Assets.img(pick);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void nextRoom() {
        if (currentRoom < rooms.length - 1) currentRoom++;
    }
