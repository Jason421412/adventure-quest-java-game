    ...
    public void paintComponent(Graphics g){
        ...
        if (currentRoom != null) {
            ...
            if (currentItem != null) {
                ...
                // 检查玩家是否与当前物品的边界相交
// Check if the player intersects with the boundary of the current item
                if (player.getBounds().intersects(new Rectangle(currentItem.getX(), currentItem.getY(), 32, 32))) {

                    // 如果当前房间有物品
                    // If the current room has an item
                    if (currentRoom.getItem() != null) {

                        // 将物品添加到库存面板中（传递物品图像和名称）
                        // Add the item to the inventory panel (passing item image and name)
